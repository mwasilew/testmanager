...
class Command(BaseCommand):
    def handle(self, *args, **options):
        for repository in TestRepository.objects.filter(is_cloned=True):
            ...
            origin.pull()
            # process only 100 last commits on master branch
            for commit in r.iter_commits('%s..HEAD' % repository.head_revision, reverse=True):
                # TODO: move this loop to helper function
                ...
                repository.head_revision = deepcopy(commit.hexsha)
                repository.save()
